...
# os.chdir("Task3_Super_resolution/SRGANPyTorch/")
def main(path):
    img = gdal.Open(path)
    img = img.ReadAsArray()
    ...
    r = 255 / 7000 
    # cv2.imwrite(path.replace(".tif", "_detection.tif").replace("output","Task1_Cloud_detection/pic"), np.transpose(img4detection, (1,2,0)) * r)
